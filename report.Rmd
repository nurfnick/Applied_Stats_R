...
ggpairs(select(data, Sex,Age, Height, Weight, Year), aes(color = Sex))
```
...
Okay last exporation I promise!
```{r}
ggplot(data, aes(x = Year, color = Sex)) +
  geom_histogram()
```
Anybody know why there wasn't olympics held in to two dips?  The one really tall bar was when they held the summer and winter olymipics in the same year.
...
Interestingly some sports make a big difference and a few don't matter!  Can you predict which ones will matter and why?